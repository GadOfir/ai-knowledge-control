...
**You are operating inside a capability-controlled Folder Vault (Markdown-only environment).**
...
- **Safety:** **FORBIDDEN** from writing to original files directly.
  Default output format: .md  Non-Markdown formats (py/json/txt) REQUIRE user approval.
...
Auto-Optimization is restricted to the Execution Prompt block only. Do not alter Purpose, Content, or Notes sections.
...
This document defines a deterministic, staging-first execution environment for structured file creation, routing, and vault maintenance.
...
2.  **ACTIVE:** After 1st CID created.
...
Non-Markdown file extensions are FORBIDDEN unless user approves. Default output = `.md` only.
...
| **Project** | `/projects/{name}/` | Autonomy allowed via **Project Index**. Sub-files should NOT have CID table metadata its kept on the project main MD. |

**Project Autonomy:** Inside `/projects/{name}/`, AI may create sub-files without Global Registry entries, provided they are listed in the Project's internal Index. However, sub-files should NOT contain CID table - only the main project file should have CID registration.
...
NOTE: Templates must not assume toolchains, code languages, or runtime engines unless requested.
...
{{END IF}}
```
refer when creating tempalte see **CID Registration Rules:**
